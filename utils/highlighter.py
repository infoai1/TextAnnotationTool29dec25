...
# PERFORMANCE: Pre-compile default keyword patterns at module load
_DEFAULT_KEYWORD_PATTERNS = {
    kw: re.compile(re.escape(kw), re.IGNORECASE)
    for kw in DEFAULT_KEYWORDS
}

# Cache for custom keyword patterns to avoid recompilation
_KEYWORD_PATTERN_CACHE: Dict[str, Any] = {}


def _get_keyword_pattern(keyword: str):
    """Get or create compiled pattern for keyword (cached)."""
    if keyword not in _KEYWORD_PATTERN_CACHE:
        _KEYWORD_PATTERN_CACHE[keyword] = re.compile(re.escape(keyword), re.IGNORECASE)
...
def find_keyword_positions(text: str, keywords: List[str]) -> List[Tuple[int, int, str]]:
    ...
    # Find keywords (case-insensitive) using cached patterns
    for keyword in keywords:
        if not keyword.strip():
            continue
        # Use pre-compiled pattern if available, otherwise get from cache
        if keyword in _DEFAULT_KEYWORD_PATTERNS:
            pattern = _DEFAULT_KEYWORD_PATTERNS[keyword]
        else:
