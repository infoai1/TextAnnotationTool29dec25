...
def load_saved_book(fpath):
    ...
    # PERFORMANCE: Use lookup dict instead of O(n²) nested loops
    if st.session_state.groups:
        para_lookup = {p['id']: p for p in st.session_state.paragraphs}
        for group in st.session_state.groups:
            group_id = group['group_id']
            for para_id in group.get('para_ids', []):
                if para_id in para_lookup:
                    para_lookup[para_id]['group_id'] = group_id
    else:
        ...
        try:
            ...
            if st.session_state.paragraphs:
                ...
                if already_matched:
                    ...
                    # Track stats incrementally (O(n) instead of O(n²))
                    stats = {'exact': 0, 'fuzzy': 0, 'estimated': 0, 'last_idx': -1}
                    ...
                    def update_progress(current, total):
                        ...
                        # PERFORMANCE: Incremental stat tracking - only check newly processed paragraphs
                        for idx in range(stats['last_idx'] + 1, current):
                            match_type = st.session_state.paragraphs[idx].get('page_info', {}).get('match_type')
                            if match_type == 'exact':
                                stats['exact'] += 1
                            elif match_type == 'fuzzy':
                                stats['fuzzy'] += 1
                            elif match_type == 'estimated':
                                stats['estimated'] += 1
                        stats['last_idx'] = current - 1
        ...
        if PDF_SUPPORT and st.session_state.pdf_pages:
            try:
                ...
                if needs_matching:
                    ...
                    # Use incremental stats for final count
                    status.update(label=f"✅ Matched to PDF! {stats['exact']} exact, {stats['fuzzy']} fuzzy, {stats['estimated']} estimated", state="complete")
                else:
                ...
                with col_rematch:
                    if st.button("🔁 Re-match", key='rematch_pdf', help="Re-run paragraph-to-PDF matching"):
                        ...
                        with st.status("🔄 Re-matching paragraphs to PDF...", expanded=True) as status:
                            ...
                            # Use incremental stats for final count
                            status.update(label=f"✅ Re-matched! {stats['exact']} exact, {stats['fuzzy']} fuzzy, {stats['estimated']} estimated", state="complete")
