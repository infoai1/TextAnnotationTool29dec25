...
@st.cache_data(show_spinner=False)
def cached_extract_pdf_pages(file_bytes: bytes):
    """Cached wrapper for PDF extraction."""
    if PDF_SUPPORT:
        return extract_pdf_pages(BytesIO(file_bytes))
    return []
...
def save_progress():
    ...
    if st.session_state.book_title and st.session_state.paragraphs:
        slug = st.session_state.book_title.lower().replace(' ', '_').replace('/', '_')[:50]
...
def save_last_read_position(para_id):
    """Save the user's last reading position for the current book."""
    book_folder = st.session_state.get('current_book_folder')
    user = st.session_state.get('current_user', 'guest')
    if book_folder and para_id:
        try:
            filepath = os.path.join(book_folder, f'last_read_{user}.json')
            with open(filepath, 'w') as f:
                json.dump({'para_id': para_id, 'timestamp': datetime.now().isoformat()}, f)
        except Exception:
            pass  # Silent fail for reading progress
...
def calculate_progress(paragraphs):
    """Calculate annotation progress 0-100%."""
    if not paragraphs:
        return 0
    reviewed = sum(1 for p in paragraphs if p.get('reviewed'))
    return int((reviewed / len(paragraphs)) * 100)
...
def load_book_from_library(book_folder):
    ...
    # Load PDF for page matching (skip if already loaded from saved progress)
    if os.path.exists(pdf_path) and not st.session_state.get('pdf_loaded', False):
...
def return_for_revision(book_folder):
    """Return book for revision (reviewer action)."""
    meta = load_meta(book_folder)
    meta['status'] = 'pending'
    meta['locked_by'] = None
    meta['locked_at'] = None
...
def process_uploaded_file(uploaded_file):
    ...
    # Extract footnotes from DOCX (Word's footnote feature)
    file_content.seek(0)
    ...
    # Match paragraphs to PDF pages if PDF already loaded
...
def toggle_page_edit(para_id):
    """Callback to toggle page edit mode."""
    edit_key = f"edit_page_{para_id}"
    st.session_state[edit_key] = not st.session_state.get(edit_key, False)


def toggle_group(para_id):
    """Callback to toggle paragraph grouping."""
    if para_id in st.session_state.selected_for_grouping:
...
def render_paragraph(para_idx: int):
    ...
    with st.container():
        ...
        if not st.session_state.get(add_ref_key, False):
            if st.button("➕ Add Reference", key=f"btn_add_ref_{para_id}", help="Add Quran, Hadith, Seerah or other reference"):
            ...
            with close_col:
                if st.button("✖", key=f"close_add_ref_{para_id}", help="Close"):
    ...
    with back_col:
        if st.button("← Back to Dashboard"):
            with st.spinner("Returning..."):
                ...
                save_progress()
                # Clear book state to return to portal
                st.session_state.file_uploaded = False
